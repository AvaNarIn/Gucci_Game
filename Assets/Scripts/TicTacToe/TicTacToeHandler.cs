...
public class TicTacToeHandler : ItemHandler
{
    ...
    public override IEnumerator CountingScore_Coroutine()
    {
        ...
        int[][] lines = new int[][]
        {
            new int[] {0,1,2}, // верхн€€ строка
            new int[] {3,4,5}, // средн€€ строка
            new int[] {6,7,8}, // нижн€€ строка
            new int[] {0,3,6}, // левый столбец
            new int[] {1,4,7}, // средний столбец
            new int[] {2,5,8}, // правый столбец
            new int[] {0,4,8}, // главна€ диагональ
            new int[] {2,4,6}  // побочна€ диагональ
        };
        ...
        yield return new WaitForSeconds(animationDuration); //«ј√Ћ”Ў ј ѕќƒ јЌ»ћј÷»ё

 
        float totalScore = CalculateScore(marks, positions, winningLines);
        LastScore = totalScore;
        Debug.Log($"ќчки за крестики-нолики: {totalScore}");
    }
